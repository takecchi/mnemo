...
| 監査ログの量 | `memory_events` テーブルのサイズが MemoryStore 本体を上回るペースで伸びる | テナント単位の保持期間設定と、期限切れ削除を `purged` イベントとして残す仕組み（§17 相当）を Phase 3 で実装する。スキーマは Phase 1 から仕込んである。 |
...
| 「認知レイヤー」という位置づけが、実際には利用側のプロンプト構築と密結合になる | 呼び出し側が mnemo の返り値を丸ごとプロンプトに焼き込む実装になり、`budget` / `omitted` / `usage` を無視し始める | サンプルアプリで `budget` と `omitted` の扱いを模範として示す。「載せるかどうかを決めるのは呼び出し側の責任」であることをドキュメントで明記する。 |
...
| pgvector のバージョン要件（iterative scan は 0.8.0 以降、CVE-2026-3172 の修正は 0.8.2） | マネージド Postgres の環境で `hnsw.iterative_scan` が使えない、または既知の脆弱性が残ったバージョンが動いている | `SELECT * FROM pg_available_extensions WHERE name = 'vector'` で各環境のバージョンを確認する運用手順をドキュメントに明記する。要件を `>= 0.8.0`、推奨を `>= 0.8.2` として明文化する。**マネージド Postgres 各社が実際に提供するバージョンは確認できていない**（§5 参照）。 |
...
mnemo はこの前提を採用しない。mnemo は計測（`usage`）を提供するが、**それが肥大を抑止すると主張しない。** 抑止する力を持つのは計測ではなく、`budget` という強制力のある仕組みである（`budget` を超えたら実際に候補を落とす）。
...
**なぜオーナーが決めるべきか**: 「AI の推論を事実と並べて出す」ことへのリスク許容度は、想定する利用先（業務エージェント／個人 AI／ゲーム NPC）によって大きく異なり、mnemo 単独では決められない。

**推奨**: 既定は含める。ただし `provenance.kind` によって区別できる形で返し、除外するオプションを用意する。除外を既定にすると、AI が導出した有用な推論が最初から見えなくなり、mnemo の価値の一部（内省・統合）が届かなくなる。区別して見せる方が「文脈を剥がして提示しない」という全体の原則にも合う。
...
- 可変次元のベクトル列を Drizzle でどう表現するかの一次情報（空間ごとにテーブルを分ける設計で回避したため、mnemo には影響しない）
