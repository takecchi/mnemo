import type { Ctx, RecallBudget, RecallResult, Runtime } from "@mnemo/core";
import type { Conversation } from "./scenario.js";

export interface MnemoPathOptions {
  budget?: RecallBudget;
}

export interface MnemoPathResult {
  recall: RecallResult;
}
...
export async function runMnemoPath(
  runtime: Runtime,
  ctx: Ctx,
  conversation: Conversation,
  opts: MnemoPathOptions = {},
): Promise<MnemoPathResult> {
...
/**
 * mnemo path が実際にプロンプトへ積む文字列を、`recall()` の返り値だけから組み立てる。
 * `usage.chars` が数えているのと同じ材料（各 memory の digest + index band の JSON）を
 * 呼び出し側の視点で再現する——「mnemo はプロンプトを組み立てない」ことを実演する関数。
 */
export function buildMnemoPrompt(recall: RecallResult): string {
